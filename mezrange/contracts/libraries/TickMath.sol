...
library TickMath {
    ...
    /// @param widthBps range width in basis points (e.g. 1000 = 10%)
    ...
    {
        // Approximation: 1 tick ~= 1 basis point for the range widths used here.
        // widthBps is the total width, so divide by 2 to get the one-sided offset.
        int24 halfRange = int24(int256(uint256(widthBps) / 2));

        tickLower = roundTickDown(currentTick - halfRange, tickSpacing);
        tickUpper = roundTickDown(currentTick + halfRange, tickSpacing) + tickSpacing;
