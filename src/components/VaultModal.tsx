...
export default function VaultModal({
  ...
  const isPending = isApproving || isDepositing || waitDeposit || isRedeeming || waitRedeem;
  const txHash    = depositTxHash ?? redeemTxHash;
  const txErr     = depositErr ?? redeemErr;
  ...
  const depositDisabled =
    ...
    (isConnected && amountNum > 0 && !hasBalance);
                ...
                >
                  {!isConnected ? (
                    ...
                    <span className="flex items-center justify-center gap-2">
                      ...
                      {isApproving ? 'Approving…' : tab === 'deposit' ? 'Depositing…' : 'Withdrawing…'}
                    </span>
                  ) : tab === 'deposit' && isConnected && amountNum > 0 && !hasBalance ? (
                    <span className="flex items-center justify-center gap-2">
