...
contract MezRangeVaultTest is Test {
    ...
    function test_Deposit_BasicShareMinting() public {
        ...
        // First deposit also mints MIN_INITIAL_SHARES to the dead address as
        // the ERC-4626 inflation-attack guard, so total supply = user shares + dead.
        assertEq(vault.totalSupply(), shares + vault.MIN_INITIAL_SHARES(), "Total supply mismatch");
        assertEq(vault.balanceOf(address(0xdead)), vault.MIN_INITIAL_SHARES(), "Dead shares missing");
        vm.stopPrank();
    }
    ...
    function test_Rebalance_UpdatesRange() public {
        ...
        // Mock pool returns swap output at 1:1; admin widens slippage to keep
        // the test focused on rebalance bookkeeping, not on swap pricing math.
        vm.prank(admin);
        strategy.setSlippage(500);
    ...
    function test_EdgeCase_DepositWhenOutOfRange() public {
        ...
        int24 newSpot = strategy.currentTickUpper() + 500;
        pool.setTick(newSpot);
        pool.setTickCumulatives(0, int56(newSpot) * int56(int32(300)));
    ...
    function test_EdgeCase_TWAP_FallbackToSpot() public {
        ...
        // Mock pool returns 1:1 swap output; widen slippage for the test.
        vm.prank(admin);
        strategy.setSlippage(500);
    ...
    /// @notice Pausing the vault must NOT trap user funds. Withdraw/redeem
    ///         remain available so a paused admin cannot indefinitely lock
    ///         depositors out of their assets.
    function test_EdgeCase_Pause_DoesNotBlockWithdraw() public {
        _depositAndActivatePosition();
        ...
        vm.prank(alice);
        uint256 out = vault.redeemWithMinAssets(aliceShares, 0);
        assertGt(out, 0, "Withdraw must succeed while vault is paused");
        assertEq(vault.balanceOf(alice), 0, "Shares burned");
    }

    /// @notice Regression for the ERC-4626 inflation/donation attack.
    ///         An attacker who deposits the minimum + donates a large amount
    ///         to the strategy must not be able to make subsequent depositors
    ///         round to zero shares.
    function test_Security_InflationAttackBlocked() public {
        // Attacker deposits exactly the minimum to mint shares.
        uint256 attackerDeposit = vault.MIN_INITIAL_SHARES();
        vm.startPrank(alice);
        token0.approve(address(vault), attackerDeposit);
        uint256 attackerShares = vault.deposit(attackerDeposit, alice);
        vm.stopPrank();
        assertEq(attackerShares, vault.MIN_INITIAL_SHARES(), "Attacker got expected shares");

        // Attacker tries to inflate share price by donating MUSD to the strategy.
        token0.mint(address(strategy), 1_000e18);

        // Victim deposits a reasonable amount; must still receive shares > 0.
        vm.startPrank(bob);
        token0.approve(address(vault), DEPOSIT_AMOUNT);
        uint256 victimShares = vault.deposit(DEPOSIT_AMOUNT, bob);
        vm.stopPrank();
        assertGt(victimShares, 0, "Victim must receive nonzero shares");
    }
    ...
    function test_FailedSwap_RebalanceReverts() public {
        ...
        // Force the pool's swap to revert on the next swap (post-router-removal
        // path: swaps go directly through pool.swap, not the router).
        pool.setSwapReverts(true);

        // The rebalance must revert entirely because the token-ratio swap fails
        vm.prank(keeper);
        vm.expectRevert("MockPool: swap failed");
        strategy.rebalance();
    ...
    function testFuzz_Deposit_SharesMinted(uint256 amount) public {
        // Bound minimum to MIN_INITIAL_SHARES (1e12) so the initial-deposit guard passes.
        amount = bound(amount, 1e12, 50_000e18);
    ...
    function testFuzz_TwoDepositors_ProportionalShares(uint256 amtAlice, uint256 amtBob) public {
        // Use larger minimums so dead-share dilution (1e12 shares to 0xdead on first
        // deposit) is negligible relative to deposit amounts — proportionality holds
        // only when deposits >> MIN_INITIAL_SHARES.
        amtAlice = bound(amtAlice, 1e15, 10_000e18);
        amtBob   = bound(amtBob,   1e15, 10_000e18);
